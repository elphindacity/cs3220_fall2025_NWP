...
class CrazyHouseEnvironment(Environment):
        ...
        if not self.is_done():
            for agent in self.agents:
                if agent.alive:
                    ...
                    print(f"Agent percepted {percept}.")
    ...
    def run(self, steps=10):
        ...
        for step in range(steps):
            if self.is_done():
                ...
                return
            print(f"*** Crazy House Step {step+1} ***")
            self.display()
    ...
    def mouse_behaviour(self):
        if self.mouse_run :
            print("Mouse ran from dog!")
            self.mouse_run = False
        if self.mouse_drink :
            print("Mouse drank the milk!")
            self.mouse_drink = False
    ...
    def execute_action(self, agent, action):
        ...
        if self.is_agent_alive(agent):
            ...
            # Track performance.
            if action == 'MoveRight':
                print("Cat moves right.")
                agent.location += 1
                agent.performance -= 1
            elif action == 'MoveLeft':
                print("Cat moves left.")
                agent.location -= 1
                agent.performance -= 1
            elif action == 'Eat':
                ...
                if agent.performance >= 10 :
                    print("Cat fights dog and wins!")
                    agent.performance += 20
                else :
                    print("Cat fights dog and loses!")
                    agent.performance -= 10
    ...
    def default_location(self, thing):
        ...
        #print(f"{thing} is starting in random room : {location}")
        return location
